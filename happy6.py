...
def read_gdp_data(happy_dict):  
    ...
    ## reprints first line with  Happiness added at end
    ...
    for line in file:
        ...
        line = line.replace(",", "")
        ### strips new line function due to adding happy_index
        line = line.strip("\n")
        ...
        gdp = gdp.replace(",","")
        ## if country's pulation is less then 100, due to commas being replaced with . 
        if (float(population)>=100):
            ## if country is present in happy_dict dictionary
            if country in happy_dict.keys():
                ## makes happy the variable for the happy index of corelated country
                happy = happy_dict[country]
                ## prints each line as country,population,gdp and now happy_index
                print(country+","+population+","+gdp +","+ happy)
